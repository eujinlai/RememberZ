...
PouchNotesObj.prototype.savenote = function () {
    ...
    */
    ...
    o.date = (this.formobject.date.value == '') ?  new Date().getTime() : new Date(this.formobject.date.value);
    	...
    	if (error) {
        	...
        	fields.map( function (f) {
        	    if (noteform[f] !== undefined && noteform[f].type != 'file') {
        	        if (noteform[f].id == 'date') {
        	            noteform[f].value = FormatDate(response[f]);
        	        } else {
...
PouchNotesObj.prototype.addrow = function (obj) {
    ...
    note = created.cloneNode();
    note.innerHTML = obj.doc.note;

    date = created.cloneNode();
    date.innerHTML = this.builddate(+obj.doc.date);

    datediff = created.cloneNode();

    var CurrentDate = new Date();
    var sdt = new Date(obj.doc.date);
    //var difdt = new Date(new Date() - sdt);
    //
    //if (sdt.getMonth() == 0) {
    //    datediff.innerHTML = (difdt.toISOString().slice(0, 4) - 1970) + " years " + (difdt.getMonth() + 1) + " months " + (difdt.getDate() - 1) + " days";
    //} else {
    //    datediff.innerHTML = (difdt.toISOString().slice(0, 4) - 1970) + " years " + (difdt.getMonth()) + " months " + (difdt.getDate() - 1) + " days";
    //}
    ...
    datediff.innerHTML = diffDuration.years() + " years " + diffDuration.months() + " months " + diffDuration.days() + " days";

     

    tr.appendChild(note);
    tr.appendChild(date);
    tr.appendChild(datediff);
